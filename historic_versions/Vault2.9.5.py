...
LOCKOUT_INFO_FILE = os.path.join(VAULT_META_DIR, "lockout_info.txt")
...
def read_lockout_info():
    """Read lockout information from the lockout info file."""
    if not os.path.exists(LOCKOUT_INFO_FILE):
        return 0, 0
    with open(LOCKOUT_INFO_FILE, "r") as f:
        lines = f.readlines()
        if len(lines) < 2:
            return 0, 0
...
def write_lockout_info(lockout_time, lockout_duration):
    """Write lockout information to the lockout info file."""
    with open(LOCKOUT_INFO_FILE, "w") as f:
        f.write(f"{lockout_time}\n{lockout_duration}\n")
        ...
        if not os.path.exists(PUBLIC_KEY_FILE):
            ...
            if encrypt_choice == "Yes":
                while True:
                    ...
                    valid, message = validate_password(pwd[0])
                    if not valid:
                        msgbox(message)
                        continue
                    if pwd[0] == pwd[1]:
